...
const LIVE_FEED = [
  ...
  },
  {
    ...
    detail: "Proxy Pickup · ID + PIN Confirmed via MoneyGram",
    time: "5m ago",
    tone: "ok" as const,
  },
  {
...
const WORKFLOW_STEPS = [
  ...
  { icon: "hub", title: "SDP Reconciliation", detail: "Deduplication and fraud analysis" },
  { icon: "key", title: "PIN-Verified Release", detail: "Tamper-evident Stellar anchoring" },
  { icon: "currency_exchange", title: "Off-Ramp Settlement", detail: "MoneyGram converts to local cash" },
];

const MAP_LEGEND = [
  { label: "Distribution Active", tone: "bg-secondary" },
  { label: "Queueing Offline", tone: "bg-tertiary" },
  { label: "Incident Area", tone: "bg-error" },
];

const STATS = [
  { label: "Total Disbursed", value: "$1.42M", icon: "payments", tag: "↑ 12% MoM", tagTone: "text-secondary bg-secondary-container/30" },
  { label: "Active Recipients", value: "12,840", icon: "groups", tag: "4 zones", tagTone: "text-on-surface-variant bg-surface-container-high" },
  { label: "Success Rate", value: "99.2%", icon: "verified", tag: "Secure", tagTone: "text-secondary bg-secondary-container/30" },
  { label: "Flagged Events", value: "03", icon: "warning", tag: "Review", tagTone: "text-error bg-error-container/30" },
];
...
export default function DeliveryDisbursementMonitorPage() {
  return (
    <AdminShell
      ...
      subtitle="Monitoring last-mile delivery across Turkana West programmes"
      mobileNavItems={MOBILE_NAV_ITEMS}
      mobileActive="deliveries"
    >
      <div className="max-w-container-max mx-auto w-full space-y-stack-lg">
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="md:col-span-4 flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
            <div>
              <h1 className="text-headline-lg font-headline-lg text-primary hidden md:block">
                Disbursement Oversight
              </h1>
              <p className="text-body-md text-on-surface-variant">
                Every batch is verified by the SDP before an off-ramp partner releases cash.
              </p>
            </div>
          ...
          {STATS.map((stat) => (
            <div key={stat.label} className="card-surface rounded-2xl p-5 flex flex-col justify-between h-32 relative overflow-hidden group">
              <div className="absolute -right-4 -top-4 text-primary opacity-[0.04] group-hover:scale-110 transition-transform">
              ...
              <div>
                ...
                <span className={`text-[10px] font-bold ml-2 px-2 py-0.5 rounded-full ${stat.tagTone}`}>{stat.tag}</span>
              </div>
            </div>
          ))}
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-stack-lg">
          ...
          <div className="lg:col-span-2 space-y-stack-md">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button className="focus-ring p-2 bg-white border border-outline-variant rounded-full hover:bg-surface-variant">
                ...
                <tbody className="text-body-md">
                  {BATCHES.map((batch) => (
                    <tr key={batch.id} className="hover:bg-surface-container-low/50 transition-colors">
                      ...
                      <td className="px-6 py-5">
                        <button
                          className={`focus-ring text-[10px] font-bold px-4 py-1.5 rounded-full active:scale-95 transition-all uppercase ${ACTION_STYLES[batch.status]}`}
                        >
                          {batch.action}
                        </button>
                      </td>
          ...
          <div className="space-y-stack-md">
            ...
            <div className="card-surface rounded-2xl aspect-[4/5] relative overflow-hidden group">
              ...
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md p-3 rounded-xl border border-outline-variant shadow-lg max-w-[160px]">
                <div className="space-y-2">
                  {MAP_LEGEND.map((item) => (
                    <div key={item.label} className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${item.tone}`} />
                      <span className="text-[10px] font-bold text-on-surface-variant uppercase">{item.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3 rounded-xl border border-outline-variant shadow-lg flex items-center justify-between">
                <div>
            ...
            {/* Quick Process View */}
            <div className="card-surface-flat rounded-2xl p-5 space-y-4">
              ...
              <button className="focus-ring w-full mt-2 py-3 bg-white border border-primary text-primary text-[10px] font-bold rounded-full uppercase tracking-wider hover:bg-primary hover:text-white transition-all">
                View Full Protocol
              </button>
            </div>
          </div>
