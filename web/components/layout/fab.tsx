...
export function Fab({ icon, label, href }: { icon: string; label: string; href: string }) {
  return (
    <Link
      href={href}
      className="focus-ring fixed bottom-28 right-5 md:bottom-8 md:right-8 w-14 h-14 bg-primary text-on-primary rounded-full elevate-lg flex items-center justify-center hover:scale-105 hover:bg-primary-container active:scale-95 transition-all z-20 group"
    >
